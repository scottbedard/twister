...
const FACE_INDEX: Record<DodecaminxFace, number> = {
  b: 0,
  bl: 1,
  br: 2,
  d: 3,
...
const bl = FACE_INDEX.bl
const br = FACE_INDEX.br
const d = FACE_INDEX.d
const dbl = FACE_INDEX.dbl
const dbr = FACE_INDEX.dbr
const dl = FACE_INDEX.dl
const dr = FACE_INDEX.dr
...
function toSimpleState(model: Dodecaminx): SimpleState {
  ...
  const faces: DodecaminxFace[] = ['b', 'bl', 'br', 'd', 'dbl', 'dbr', 'dl', 'dr', 'f', 'l', 'r', 'u']

  for (const face of faces) {
    const comp = model.state[face]
    const mapSticker = (s: DodecaminxSticker) => FACE_INDEX[s.face]
    const corners = comp[0].map(row => row.map(mapSticker))
    if (comp.length === 1) {
      out[face] = [corners]
    }
    else {
      const middles = (comp[1] ?? []).map(row => row.map(x => (x != null ? mapSticker(x) : undefined)))
      const center = comp[2] != null ? mapSticker(comp[2] as DodecaminxSticker) : undefined
      out[face] = [corners, middles, center!]
    }
  }
...
describe('dodecaminx turns', () => {
  ...
  tests.forEach((test, i) => {
    ...
    run(`#${i + 1} ${test.turn} (size ${test.size})`, () => {
      const model = new Dodecaminx(test.size)
      model.turn(test.turn)
      const actual = toSimpleState(model)
